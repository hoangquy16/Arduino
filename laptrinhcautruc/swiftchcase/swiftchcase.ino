      ...
      switch (gia_tri) {
        ...
        case M_TEN_LEN:
          if (cheDoHienTai == CHE_DO_DEN) {
            if (trangThaiDen) {
              doSangDo += 25; if (doSangDo > 255) doSangDo = 255;
              analogWrite(chan_den_do, doSangDo);
              Serial.print("Đèn Đỏ: Tăng -> "); Serial.println(doSangDo);
            }
        ...
        case M_TEN_XUONG:
          if (cheDoHienTai == CHE_DO_DEN) {
            if (trangThaiDen) {
              doSangDo -= 25; if (doSangDo < 0) doSangDo = 0;
              analogWrite(chan_den_do, doSangDo);
              Serial.print("Đèn Đỏ: Giảm -> "); Serial.println(doSangDo);
            }
          } else if (cheDoHienTai == CHE_DO_NHAC) {
            mp3.volumeDown();
        ...
        case M_TEN_TRAI:
          if (cheDoHienTai == CHE_DO_DEN) {
            if (trangThaiDen) {
              doSangXanh -= 25; if (doSangXanh < 0) doSangXanh = 0;
              analogWrite(chan_den_xanh, doSangXanh);
              Serial.print("Đèn Xanh Blue: Giảm -> "); Serial.println(doSangXanh);
            }
          } else if (cheDoHienTai == CHE_DO_NHAC) {
            mp3.previous();
        ...
        case M_TEN_PHAI:
          if (cheDoHienTai == CHE_DO_DEN) {
            if (trangThaiDen) {
              doSangXanh += 25; if (doSangXanh > 255) doSangXanh = 255;
              analogWrite(chan_den_xanh, doSangXanh);
              Serial.print("Đèn Xanh Blue: Tăng -> "); Serial.println(doSangXanh);
            }
          } else if (cheDoHienTai == CHE_DO_NHAC) {
            mp3.next();
        ...
        case PHIM_THANG:
          if (cheDoHienTai == CHE_DO_DEN) {
            ...
            if (trangThaiDen) {
              if (doSangDo == 0 && doSangXanh == 0) { doSangDo = 150; doSangXanh = 150; }
              analogWrite(chan_den_do, doSangDo);
              analogWrite(chan_den_xanh, doSangXanh);
